...
describe("Unit Testing", () => {
  ...
  describe("If is even number", () => {
    it("`isEvenNumber` should returns true", () => {
      const checkEven = isEvenNumber(6);
      expect(checkEven).to.be.true;
    });

    it("`isEvenNumber` should returns false", () => {
      const checkEven = isEvenNumber(5);
      expect(checkEven).to.be.false;
    });
  });
